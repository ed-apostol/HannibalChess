...
void Search::initNode(Thread& sthread) {
	...
	else if (!(sthread.nodes & 4095)) mEngine.CheckTime();
}
...
int Search::searchGeneric(position_t& pos, int alpha, int beta, const int depth, SearchStack& ssprev, Thread& sthread) {
	...
	while ((move = sortNext(sp, mInfo, pos, *ss.mvlist, sthread)) != nullptr) {
		...
		if (!inSplitPoint && !inSingular && !sthread.stop && sthread.num_sp < mInfo.mMaxActiveSplitsPerThread && mEngine.ThreadNum() > 1
			&& depth >= mInfo.mMinSplitDepth && (inRoot || inPv || !sthread.activeSplitPoint || !sthread.activeSplitPoint->workAvailable
				|| sthread.activeSplitPoint->depth - depth <= 1)) {
...
void Engine::CheckTime() {
	// allow all threads to check for timeout as the main thread might get trapped
	// in the IdleLoop waiting for other threads to finish thereby not reaching this
	int64 time2 = getTime();

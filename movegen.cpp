...
void genGainingMoves(const position_t *pos, movelist_t *mvlist, int delta, int thread_id) {
    ...
    if (pos->side == BLACK) { //TODO consider writing so we don't need this expensive branch
        if (ThreadsMgr.ThreadFromIdx(thread_id).evalgains[historyIndex(pos->side, GenBlackOO())] >= delta && (pos->posStore.castle&BCKS) && (!(occupied&(F8|G8)))) {
            if (!isAtt(pos, pos->side^1, E8|F8|G8))
                mvlist->list[mvlist->size++].m = GenBlackOO();
        }
        if (ThreadsMgr.ThreadFromIdx(thread_id).evalgains[historyIndex(pos->side, GenBlackOOO())] >= delta && (pos->posStore.castle&BCQS) && (!(occupied&(B8|C8|D8)))) {
        ...
        if (ThreadsMgr.ThreadFromIdx(thread_id).evalgains[historyIndex(pos->side, GenWhiteOO())] >= delta && (pos->posStore.castle&WCKS) && (!(occupied&(F1|G1)))) {
            if (!isAtt(pos, pos->side^1, E1|F1|G1))
                mvlist->list[mvlist->size++].m = GenWhiteOO();
        }
        if (ThreadsMgr.ThreadFromIdx(thread_id).evalgains[historyIndex(pos->side, GenWhiteOOO())] >= delta && (pos->posStore.castle&WCQS) && (!(occupied&(B1|C1|D1)))) {
    ...
    while (pc_bits_p1) {
        ...
        while (mv_bits) {
            to = popFirstBit(&mv_bits);
            if (ThreadsMgr.ThreadFromIdx(thread_id).evalgains[historyIndex(pos->side, GenOneForward(from, to))] >= delta)
                mvlist->list[mvlist->size++].m = GenOneForward(from, to);
        }
    }
    ...
    while (pc_bits_p2) {
        ...
        while (mv_bits) {
            to = popFirstBit(&mv_bits);
            if (ThreadsMgr.ThreadFromIdx(thread_id).evalgains[historyIndex(pos->side, GenTwoForward(from, to))] >= delta)
                mvlist->list[mvlist->size++].m = GenTwoForward(from, to);
        }
    }
    ...
    while (pc_bits) {
        ...
        while (mv_bits) {
            to = popFirstBit(&mv_bits);
            if (ThreadsMgr.ThreadFromIdx(thread_id).evalgains[historyIndex(pos->side, GenKnightMove(from, to, EMPTY))] >= delta)
    ...
    while (pc_bits) {
        ...
        while (mv_bits) {
            to = popFirstBit(&mv_bits);
            if (ThreadsMgr.ThreadFromIdx(thread_id).evalgains[historyIndex(pos->side, GenBishopMove(from, to, EMPTY))] >= delta)
    ...
    while (pc_bits) {
        ...
        while (mv_bits) {
            to = popFirstBit(&mv_bits);
            if (ThreadsMgr.ThreadFromIdx(thread_id).evalgains[historyIndex(pos->side, GenRookMove(from, to, EMPTY))] >= delta)
    ...
    while (pc_bits) {
        ...
        while (mv_bits) {
            to = popFirstBit(&mv_bits);
            if (ThreadsMgr.ThreadFromIdx(thread_id).evalgains[historyIndex(pos->side, GenQueenMove(from, to, EMPTY))] >= delta)
    ...
    while (pc_bits) {
        ...
        while (mv_bits) {
            to = popFirstBit(&mv_bits);
            if (ThreadsMgr.ThreadFromIdx(thread_id).evalgains[historyIndex(pos->side, GenKingMove(from, to, EMPTY))] >= delta)
